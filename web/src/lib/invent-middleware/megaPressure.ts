...
interface SurvivalETAPrediction {
  rounds_eta: number;
  time_eta_minutes: number;
  hazard_rate: number;
  confidence_50: { min_rounds: number; max_rounds: number; min_time: number; max_time: number };
  ...
  confidence_95: { min_rounds: number; max_rounds: number; min_time: number; max_time: number };
  methodology: string;
  survival_curve: Array<{ round: number; survival_probability: number }>;
}
...
interface SemanticProbability {
  band: string;
  probability_mass: number;
  strategic_function: string;
}

interface SemanticRangePrediction {
  predicted_range: { min: number; max: number };
  confidence_intervals: {
    p50: { min: number; max: number };
    p75: { min: number; max: number };
  ...
  probability_distribution: Record<string, number>;
  semantic_weights: SemanticProbability[];
  historical_accuracy: number;
  methodology: string;
}

interface DNAMatch {
  historical_round_id: number;
  similarity_score: number;
  distance_metric: number;
  matched_multiplier: number;
  matched_gap: number;
  time_delta_minutes: number;
}

interface DNASimilarityResult {
  current_state: Record<string, any>;
  top_matches: DNAMatch[];
  predicted_multiplier_range: { min: number; max: number };
  confidence_score: number;
  methodology: string;
}

interface EVGuardrail {
...
interface EnhancedChaseStrategy {
  strategy: string;
  base_bet: number;
...
interface BrierScoreResult {
  brier_score: number;
  calibration_error: number;
  resolution: number;
  uncertainty: number;
  reliability_diagram: Array<{ bin: string; predicted_prob: number; actual_freq: number; count: number }>;
  sample_size: number;
  methodology: string;
}
...
class MegaPressureAnalyzer {
  ...
  /**
   * Calculate ETA prediction with confidence intervals (legacy method)
   */
  ...
  /**
   * Calculate survival-based ETA prediction (Kaplan-Meier)
   */
  async calculateSurvivalETA(source: string, range: { min: number; max: number }, fullscreen: boolean = false): Promise<SurvivalETAPrediction> {
    const api = await import('@/lib/api');
    const targetMultiplier = range.max === Infinity ? null : range.max;
    const targetParam = targetMultiplier ? `&target_multiplier=${targetMultiplier}` : '';
    const response = await api.api.request<SurvivalETAPrediction>(`/mega-pressure/eta?source=${source}${targetParam}&fullscreen=${fullscreen}`);
    return response;
  }

  /**
   * Calculate semantic range prediction
   */
  async calculateSemanticRange(source: string, range: { min: number; max: number }, fullscreen: boolean = false): Promise<SemanticRangePrediction> {
    const api = await import('@/lib/api');
    const response = await api.api.request<SemanticRangePrediction>(`/mega-pressure/range?source=${source}&min_multiplier=${range.min}`);
    return response;
  }

  /**
   * Calculate DNA similarity for extreme target prediction
   */
  async calculateDNASimilarity(source: string, targetMultiplier: number | null = null, windowSize: number = 20): Promise<DNASimilarityResult> {
    const api = await import('@/lib/api');
    const targetParam = targetMultiplier ? `&target_multiplier=${targetMultiplier}` : '';
    const response = await api.api.request<DNASimilarityResult>(`/mega-pressure/dna-similarity?source=${source}${targetParam}&window_size=${windowSize}`);
    return response;
  }

  /**
   * Calculate enhanced chase strategy with EV guardrails
   */
  async calculateEnhancedChase(source: string, strategy: string = 'moderate', bankroll: number = 1000, targetMultiplier: number = 50): Promise<EnhancedChaseStrategy> {
    const api = await import('@/lib/api');
    const response = await api.api.request<EnhancedChaseStrategy>(`/mega-pressure/enhanced-chase?source=${source}&strategy=${strategy}&bankroll=${bankroll}&target_multiplier=${targetMultiplier}`);
    return response;
  }
   ...
   */
  async calculateBrierScore(source: string, minMultiplier: number = 50, testSize: number = 100): Promise<BrierScoreResult> {
    const api = await import('@/lib/api');
    const response = await api.api.request<BrierScoreResult>(`/mega-pressure/brier-score?source=${source}&min_multiplier=${minMultiplier}&test_size=${testSize}`);
    return response;
  }
   ...
   */
  async calculateForwardTest(source: string, minMultiplier: number = 50, testRounds: number = 500): Promise<ForwardTestResult> {
    const api = await import('@/lib/api');
    const response = await api.api.request<ForwardTestResult>(`/mega-pressure/forward-test?source=${source}&min_multiplier=${minMultiplier}&test_rounds=${testRounds}`);
    return response;
  }
...
export function useSurvivalETA(source: string, range: { min: number; max: number }, fullscreen: boolean = false) {
  return useQuery({
    queryKey: ['survival-eta', source, range.min, range.max, fullscreen],
    queryFn: () => megaPressureAnalyzer.calculateSurvivalETA(source, range, fullscreen),
    refetchInterval: POLL_INTERVAL * 3,
    staleTime: 30000,
  });
}

export function useSemanticRange(source: string, range: { min: number; max: number }, fullscreen: boolean = false) {
  return useQuery({
    queryKey: ['semantic-range', source, range.min, range.max, fullscreen],
    queryFn: () => megaPressureAnalyzer.calculateSemanticRange(source, range, fullscreen),
    refetchInterval: POLL_INTERVAL * 3,
    staleTime: 30000,
  });
}

export function useDNASimilarity(source: string, targetMultiplier: number | null = null, windowSize: number = 20) {
  return useQuery({
    queryKey: ['dna-similarity', source, targetMultiplier, windowSize],
    queryFn: () => megaPressureAnalyzer.calculateDNASimilarity(source, targetMultiplier, windowSize),
    refetchInterval: POLL_INTERVAL * 5,
    staleTime: 60000,
  });
}

export function useEnhancedChase(source: string, strategy: string = 'moderate', bankroll: number = 1000, targetMultiplier: number = 50) {
  return useQuery({
    queryKey: ['enhanced-chase', source, strategy, bankroll, targetMultiplier],
    queryFn: () => megaPressureAnalyzer.calculateEnhancedChase(source, strategy, bankroll, targetMultiplier),
    refetchInterval: POLL_INTERVAL * 3,
    staleTime: 30000,
  });
}

export function useBrierScore(source: string, minMultiplier: number = 50, testSize: number = 100) {
  return useQuery({
    queryKey: ['brier-score', source, minMultiplier, testSize],
    queryFn: () => megaPressureAnalyzer.calculateBrierScore(source, minMultiplier, testSize),
    refetchInterval: POLL_INTERVAL * 10,
    staleTime: 120000,
  });
}

export function useForwardTest(source: string, minMultiplier: number = 50, testRounds: number = 500) {
  return useQuery({
    queryKey: ['forward-test', source, minMultiplier, testRounds],
    queryFn: () => megaPressureAnalyzer.calculateForwardTest(source, minMultiplier, testRounds),
    refetchInterval: POLL_INTERVAL * 10,
    staleTime: 120000,
  });
}

export type { MegaRound, PressureMetrics, BacktestResult, TopRound, TopRoundsData, ETAPrediction, RangePrediction, BankrollRequirement, ChaseStrategy, ChaseConfig, SurvivalETAPrediction, SemanticRangePrediction, SemanticProbability, DNASimilarityResult, DNAMatch, EVGuardrail, EnhancedChaseStrategy, BrierScoreResult, ForwardTestResult };
